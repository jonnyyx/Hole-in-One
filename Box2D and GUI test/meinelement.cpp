...
MeinElement::MeinElement(b2World *world, QGraphicsScene *level, QPointF position, qreal angle, b2BodyType type, b2CircleShape &circle)
{
    ...
    myBodyDef.angle=angle*(3.14/180); //Umrechnung von grad in rad da Angabe in Box2D in rad
...
MeinElement::MeinElement(b2World *world, QGraphicsScene *level, b2Vec2 center, qreal angle, qreal length, qreal height, b2BodyType type, qreal friction)
{
    b2PolygonShape polygon;
    polygon.SetAsBox(length/2, height/2, center, angle*(3.14/180)); //Umrechnung von angle von grad in rad
    b2BodyDef myBodyDef;
    myBodyDef.type=type; // Unterscheidung zwischen Dynamic, Static and Kinematic Body
...
//  poly->setVisible(true);
    ...
    int y=center.y-height/4;

    QRectF polyf(QPoint(x,y),QSize(length,height));
    graphics = level->addRect(polyf);
    graphics->setFlag(QGraphicsItem::ItemIsMovable,true);

    drawRec(x+length/2,y+height/2);
 ...
 *
 */
MeinElement::MeinElement(b2World *world, QGraphicsScene *level, b2Vec2 center, qreal length, qreal height, b2BodyType type, qreal friction)
{
    b2PolygonShape polygon;
    polygon.SetAsBox(length, height, center,0);
    b2BodyDef myBodyDef;
    myBodyDef.type=type; // Unterscheidung zwischen Dynamic, Static and Kinematic Body
    myBodyDef.active = true;
    myBodyDef.gravityScale = 1000.0;
...
// void MeinElement::drawGraphicsRect(){
//     QPointF v=graphics->pos();  //left-upper-position
//     //qDebug()<<v.x();
//     //qDebug()<<v.y();
