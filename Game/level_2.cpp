...
/*!
 * \brief Level_2::closeEvent
 * listen if QGraphicsView is closed
 */
void Level_2::closeEvent(QCloseEvent *){
    closeLevel();
}
 ...
 *  it is a new highscore write it into level.txt with score time ande object#
 */
void Level_2::saveLevel()
{
    ...
    if(levelenab.size()>8){
        if(levelenab.at(10).toInt()<highscore){
                ...
                newhighscore=true;
            }else if(levelenab.at(10).toInt()==highscore){
...
void Level_2::showLevel(){
     ...
     ball  = new Paperball(myWorld2, level2, QPointF(540.0,46.0), 0*(3.14/180.0), b2_dynamicBody, circle);
